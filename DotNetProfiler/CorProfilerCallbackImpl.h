...
class CCorProfilerCallbackImpl : public ICorProfilerCallback7
{
public:
	...
	// ICorProfiler6
	STDMETHOD(GetAssemblyReferences)(const WCHAR *wszAssemblyPath, ICorProfilerAssemblyReferenceProvider *pAsmRefProvider);

	//ICorProfiler7
	STDMETHOD(ModuleInMemorySymbolsUpdated)(ModuleID moduleId);

	// ICorProfiler8
	STDMETHOD(DynamicMethodJITCompilationStarted)(FunctionID functionId, BOOL fIsSafeToBlock, LPCBYTE pILHeader, ULONG cbILHeader);

	STDMETHOD(DynamicMethodJITCompilationFinished)(FunctionID functionId, HRESULT hrStatus, BOOL fIsSafeToBlock);
};
